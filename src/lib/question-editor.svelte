...
<div class="QuestionEditor flex flex-col gap-4 w-full">
    <div class="w-full">
        ...
        <Milkdown markdown={text} on:change={(e) => text = e.detail.value} readonly={!editable} />
    </div>
    {#each $choices as choice, index}
        <div class="input-group input-group-divider grid-cols-[auto_1fr_auto] cursor-pointer {choice.correct ? '!bg-success-500/40':''}">

            {#if editable}
                <div class="input-group-shim">{labels[index]}</div>
                <input type="text" bind:value={choice.text} placeholder="Choice Answer {labels[index]}" />
            {:else}
                <button class="input-group-shim w-[50px]" on:click={() => {
                    if (!editable) handleToggleChecked(choice)
                }}><p class="text-center w-full">{labels[index]}</p></button>
                <button class="py-3 px-4" on:click={() => {
                    if (!editable) handleToggleChecked(choice)
                }}>{choice.text}</button>
            {/if}
