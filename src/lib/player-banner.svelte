<script lang="ts">
	...
	function extractPlayerRoomData(roomData: App.Room | undefined) {
		if (!roomData?.players) return player;
		const arr = Object.entries(roomData.players).map(([_, p]) => p);
		return arr.find((p) => p.id === player.id) ?? player;
	}
