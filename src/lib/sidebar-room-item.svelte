<script lang="ts">
	...
	function countReadyPlayer(players: Record<string, App.Player>) {
		const p = Object.entries<App.Player>(players).map(([_, player]) => player);
		return p.reduce((count: number, player: App.Player) => {
...
{#each rooms as game, i}
	<li class="hover:bg-primary-500/20 hover:rounded cursor-pointer pt-1 pb-2 px-2">
		<a href="/{game.host.name}/{game.name}" data-sveltekit-reload class="flex flex-col items-start">
			...
			<p class="inline-flex gap-2 flex-wrap text-xs">
				...
				<span class="inline-flex items-center">
					...
					{:else}
						<span class="bg-black text-white px-2 py-1 rounded-s-full"
							>{countReadyPlayer(game.players ?? {})}</span
						>
