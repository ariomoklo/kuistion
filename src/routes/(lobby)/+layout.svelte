...
<div
	class="chat w-full h-full min-h-[calc(100vh-86px)] lg:min-h-screen grid grid-cols-1 lg:grid-cols-[30%_1fr] xl:grid-cols-[20%_1fr]"
>
	<!-- Navigation -->
			...
			<svelte:fragment slot="header">
				{#if data.room}
					<div class="flex flex-col w-full gap-2">
						<a href="/{data.player.name}" class="inline-flex items-center w-full gap-2">
							<IconBack class="text-xs" />
							<span>Room List</span>
						</a>

						<div
							class="bg-{data.room.status === 'in-play'
								? 'success'
								: 'secondary'}-500/50 px-2 pt-1 pb-2 w-full rounded"
						>
